...
const DFSVisualizer = () => {
  ...
  const handleVisualize = () => {
    ...
    const addStep = (line, description, visitedNodes, stackNodes, currentNode) => {
      newHighlightLines.push(line);
      newStepDescriptions.push(description);
      newSteps.push({ visited: [...visitedNodes], stack: [...stackNodes], currentNode });
    };

    // DFS Algorithm
    const visitedNodes = new Set();
    const stackNodes = [startNode];
    addStep(1, `Starting DFS from node ${startNode}`, [], stackNodes, startNode);

    while (stackNodes.length > 0) {
      const node = stackNodes.pop();
      if (!visitedNodes.has(node)) {
        visitedNodes.add(node);
        addStep(2, `Visiting node ${node}`, Array.from(visitedNodes), stackNodes, node);

        for (const neighbour of graph[node]) {
          if (!visitedNodes.has(neighbour)) {
            stackNodes.push(neighbour);
            addStep(3, `Adding node ${neighbour} to the stack`, Array.from(visitedNodes), stackNodes, node);
          }
        }
      }
    }
  ...
  const handleAnimate = (index) => {
    ...
    const { visited: visitedNodes, stack: stackNodes, currentNode } = steps[index];
    ...
    // Draw stack
    ctx.fillStyle = '#000';
    ctx.fillText(`Stack: ${stackNodes.join(', ')}`, 10, canvas.height - 20);
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        ...
        <div style={{ textAlign: 'center', color: 'red' }}>
          <h1 className="text-3xl font-bold text-center text-red-600">DEPTH-FIRST SEARCH VISUALIZATION</h1>
        </div>

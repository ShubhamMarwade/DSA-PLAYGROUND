...
const LinearSearchVisualizer = () => {
  ...
  const handleVisualize = () => {
    ...
    const addStep = (line, description, arrState, index) => {
      newHighlightLines.push(line);
      newStepDescriptions.push(description);
      newSteps.push({ arrState: [...arrState], index });
    };

    // Generate the steps for visualization
    addStep(1, 'Line 1: Define the linear_search function to search for a target in an array.', arr, null);
    for (let i = 0; i < arr.length; i++) {
      addStep(2, `Line 2: Compare the target (${tgt}) with the current element (arr[${i}] = ${arr[i]}).`, arr, i);
      if (arr[i] === tgt) {
        addStep(3, `Line 3: Target found at index ${i}.`, arr, i);
        break;
      }
    }

    if (steps.length === 0 || steps[steps.length - 1].arrState.indexOf(tgt) === -1) {
      addStep(4, `Line 4: Target not found in the array.`, arr, null);
    }
  ...
  const handleAnimate = (index) => {
    ...
    const { arrState, index: currentIndex } = steps[index];
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    const elementWidth = canvas.width / n;
    const elementHeight = 40;

    for (let i = 0; i < arrState.length; i++) {
      ctx.fillStyle = (i === currentIndex) ? '#5DE23C' : 'red'; // Highlight current element
      ...
      if (i === currentIndex) {
        ctx.fillStyle = '#5DE23C';
        ctx.beginPath();
        ctx.moveTo(i * elementWidth + elementWidth / 2, canvas.height / 2 - elementHeight / 2 - 10);
        ctx.lineTo(i * elementWidth + elementWidth / 2 - 5, canvas.height / 2 - elementHeight / 2 - 2);
        ctx.lineTo(i * elementWidth + elementWidth / 2 + 5, canvas.height / 2 - elementHeight / 2 - 2);
        ctx.closePath();
        ctx.fill();
      }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        ...
        <div style={{ textAlign: 'center', color: 'red' }}>
          <h1 className="text-3xl font-bold text-center text-red-600">LINEAR SEARCH VISUALIZATION</h1>
        </div>
